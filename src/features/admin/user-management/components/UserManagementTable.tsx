    ...
    if (serverUsers && Array.isArray(serverUsers) && serverUsers.length > 0) {
      setUsers(
        serverUsers.map((u: any) => ({
          ...
          role: (u.role === "care_company" ? "Company" : u.role === "service_provider" ? "Provider" : u.role ? u.role.charAt(0).toUpperCase() + u.role.slice(1) : "Company") as UserRole,
          joiningDate: u.joiningDate ? new Date(u.joiningDate).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) : "Recently",
          location: u.location || "UK",
          status: (u.status ? u.status.charAt(0).toUpperCase() + u.status.slice(1) : "Active") as UserStatus,
        }))
      );
    }
  }, [serverUsers]);
