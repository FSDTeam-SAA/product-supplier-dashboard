...
export default function ApprovalsTable() {
  const { data: serverApprovals } = useApprovals();
  const updateStatusMutation = useUpdateApprovalStatus();

  const [data, setData] = useState<ApprovalRecord[]>(initialApprovalData);

  React.useEffect(() => {
    if (serverApprovals && Array.isArray(serverApprovals) && serverApprovals.length > 0) {
      setData(
        serverApprovals.map((item: any) => ({
          id: item.id || item._id,
          name: item.name || item.fullName || "Unnamed",
          role: (item.role || "Company") as UserRole,
          email: item.email || "N/A",
          location: item.location || "London, UK",
          status: (item.status ? item.status.charAt(0).toUpperCase() + item.status.slice(1) : "Pending") as ApprovalStatus,
        }))
      );
    }
  }, [serverApprovals]);
