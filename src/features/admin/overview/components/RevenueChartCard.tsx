...
import { useAdminRevenueChart } from "../hooks/useAdminOverview";

export default function RevenueChartCard({
  data = initialRevenueData,
}: RevenueChartDataProps) {
  const { data: chartResponse } = useAdminRevenueChart();

  const activeData = React.useMemo(() => {
    if (chartResponse?.chartData && Array.isArray(chartResponse.chartData) && chartResponse.chartData.length > 0) {
      return {
        title: "Revenue",
        selectedPeriod: `${chartResponse.year || 2026}`,
        chartData: chartResponse.chartData.map((item: any) => ({
          month: item.month,
          revenue: item.totalRevenue,
          formattedRevenue: `$${Number(item.totalRevenue).toLocaleString()}`,
        })),
      };
    }
    return data;
    ...
    <div className="px-6">
      <Card className="rounded-2xl bg-card p-6 md:p-8 shadow-xs font-sans">
        ...
        <CardHeader className="p-0 flex flex-row items-center justify-between space-y-0 mb-8">
          ...
          >
            <span>{activeData.selectedPeriod}</span>
